...
*Last updated: **by_member** backlog — **Member by roles** table ([`byMember.js`](src/modules/app/render/backlog/byMember.js)): `split_*_pct`, **`targetCapacityRowIdByRoleId`**, legacy **`targetCapacityRowId`**; [`applyPlannedFromBacklog`](src/modules/app/services/backlogDemand.js) per-role routing. **Settings → Default % SP by roles** visible for SP + **By roles** or **By member** ([`syncSettingsDefaultRoleSplitSection`](src/modules/app/render/ui.js), [`settings.js`](src/modules/app/actions/settings.js)). **by_member** capacity — `(Per member)` + per-row metrics in [`byTeam.js`](src/modules/app/render/capacity/byTeam.js). [`sumPlannedForPeriod`](src/modules/app/services/backlogDemand.js) rounding. Backlog [`handleTableInput`](src/app.js) field `targetCapacityRowIdByRole`.*

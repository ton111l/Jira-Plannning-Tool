...
*Last updated: Backlog **by_team** — no **Team allocation (%)** or **Effective** columns; [`getTeamModeEffectiveDemand`](src/modules/app/services/backlogDemand.js). Settings modal scroll. **Import** **`estimationKind`**, [`getBacklogEstimationForPlan`](src/modules/app/render/shared/backlogHelpers.js).*

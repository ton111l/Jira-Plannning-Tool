...
Per-plan settings include: `estimationType`, `resourceGroupingType`, `jiraBaseUrl`, `estimationFieldName`, **`importEstimationFieldKind`** (last-used Jira field type in Import dialog), `lastImportJql`, `defaultWorkingDays`, **`defaultLoadPercent`** (Load % for all capacity rows, default 100; applied to every row on Settings Save), **`useBuffers`**, **`allBuffersPercent`** (total buffer % used in capacity math; sum of line items), **`bufferItems[]`** (optional `{ name, percent }` lines from **Buffer settings**; persisted for display and export; legacy plans with only `allBuffersPercent` are migrated to a single **All Buffers** row in `init()` when needed), **`capacityTableViewMode`**: `full` | `compact` (Capacity **View** — Compact **omits** **Days off** and all **Per member** cells so header `colspan` matches the body; **Per team** / **Role total** columns stay. Implemented by a second, flatter thead + fewer `td` per row, not CSS-only hide), **`backlogTableViewMode`**: `full` | `compact` (legacy `comfortable` normalized to `full` in `init()`). [`src/styles.css`](src/styles.css) sets `#capacityTable.grid-table` to `width`/`min-width: max-content` for **both** **full** and **compact** so the table stays **left-aligned** and only as wide as its columns; horizontal overflow scrolls inside `#capacityTableWrap.table-wrap`.
...
**Capacity buffer summary strip:** When **`useBuffers`** is on and at least one **`bufferItems[]`** line has a **positive** **`percent`**, [`renderCapacityChrome`](src/modules/app/render/ui.js) shows **`#capacityBuffersBar`** (`.capacity-summary-bar`) **between** **`#capacityToolbar`** and **`#capacityTableWrap`**, as a single line of `Name: X%` segments joined by ` | `. Legacy plans without `bufferItems` but with a **positive** **`allBuffersPercent`** still show **All Buffers** for that total. The overlay zero-state hides this bar with the rest of the Capacity chrome.
...
**Authoritative mode on the plan:** `planningTimeMode` is set when the plan is created (**Create Plan**: sprint checkbox → `sprint`, otherwise `quarter`), when sprint columns are applied ([`applySprintConfigToPlan`](src/app.js) → `sprint`), and when sprints are stripped ([`removeSprintsFromPlan`](src/app.js) → `quarter`, clears `useSprintsPlanning`). On load, [`init()`](src/app.js) **reconciles** `planningTimeMode` with `periods` after `kind` is normalized: any **`kind === "sprint"`** period forces **`sprint`**; absence of sprint periods with a corrupt **`sprint`** mode forces **`quarter`**. This keeps Settings UI and saved state aligned.

**Settings (sprint controls):** In [`renderSettings`](src/modules/app/render/ui.js), **Use Sprints Planning** and **Sprints settings** are shown **only** when `planningTimeMode === sprint`. For **quarter** plans they stay **hidden** (and the checkbox is forced unchecked when the dialog renders). Because `.checkbox-label` uses `display: inline-flex`, [`src/styles.css`](src/styles.css) includes **`.sprint-planning-grid [hidden] { display: none !important; }`** so the HTML **`hidden`** attribute reliably hides those controls. **Save Settings** applies sprint checkbox / `removeSprintsFromPlan` **only** if the active plan is already in **`planningTimeMode === sprint`**, so saving other fields on a quarter plan never runs sprint teardown logic.
...
Switching `quarter` ↔ `sprint` in general still implies rebuilding `periods` and remapping `periodValues` / `targetPeriodId`; do not reuse old `period.id` values without an explicit mapping step.

**Implemented today:** New plans record **`planningTimeMode`** at creation; sprint slice application and removal update **`planningTimeMode`** and flags as above. When sprint planning is applied during plan creation, the anchor quarter period is **replaced** by generated sprint periods (not kept in parallel), and related quarter keys are removed from `teamPeriodValues`/`capacityRows.periodValues`.

## 6. Import pipeline

- **Limits:** Jira Search / bridge requests cap at **`BACKLOG_MAX_ISSUES` (100)** in [`src/modules/planning/constants.js`](src/modules/planning/constants.js). Merging in [`submitImport`](src/app.js) still updates existing keys, but **new** rows are not appended when `plan.backlogRows.length` is already at the cap — user must delete rows to free slots. The import dialog states the limit in UI.
...
- **Re-import / merge:** rows match on normalized **issue key** (`normalizeBacklogIssueKey`). Existing keys get Jira-sourced **summary / status / issue type / priority** updated; **`estimation`** (and **`estimationKind`**) update **only** when the row’s stored **`estimation`** is blank after trim — if the cell already has a value, re-import leaves it unchanged so local edits are not overwritten. New keys are **appended** with full Jira data including **`estimation`** until the backlog reaches **`BACKLOG_MAX_ISSUES`** rows (then new keys from the Jira result are skipped until the user deletes rows). Rows removed locally (e.g. **Delete selected**) are simply absent from `backlogRows[]`; if the same issue appears again in a Jira import result, it is merged as a **new** row like any other missing key. Duplicate keys in stored backlog are deduped (first row wins) before merge; new rows from one import batch are registered so the same key cannot appear twice in that batch.
- **Jira field for estimates** is configured in the **Import backlog from Jira** dialog (not in Settings): the dialog includes **Jira estimation field type** (Story Points vs Man-days) **independent of** Settings **Estimation type**. The text field is the Jira field id (`plan.estimationFieldName`). **Story Points** type requires a non-empty custom field id; **Man-days** may be empty (defaults to `timeoriginalestimate` when resolving the API field). [`resolveImportEstimationFieldNameForImport`](src/app.js) / [`syncImportEstimationFieldUi`](src/app.js) use the **dialog** field type, not `plan.estimationType`. Display and [`applyPlannedFromBacklog`](src/modules/app/services/backlogDemand.js) use [`getBacklogEstimationForPlan`](src/modules/app/render/shared/backlogHelpers.js) so values **do not** appear as the wrong unit when the user switches plan **Estimation type** in Settings. Legacy rows without `estimationKind` still show `estimation` for any plan type. Manual edits set `estimationKind` to the current plan type. `plan.importEstimationFieldKind` stores the last-used import field type (dialog default).

## 7. Persistence and migration

- Load/save: [`src/modules/storage.js`](src/modules/storage.js).
- **Any new plan-level field** must be backfilled in [`src/app.js`](src/app.js) `init()` for older stored state (see existing patterns for **`planningTimeMode`** (reconciled with sprint vs quarter **periods** after `kind` normalization), `velocity`, period `kind`, `targetPeriodId`, **`targetCapacityRowId`** (empty string), **`capacityTableViewMode`**, **`backlogTableViewMode`** (`full` | `compact`, with legacy `comfortable` normalized to `full`), **`bufferItems`** (from **`allBuffersPercent`** when the array was missing), **`defaultRoleSplitPctByRoleId`** — for Story Points + **By roles** or **By member**, missing or incomplete defaults are replaced with [`buildEqualDefaultRoleSplitPctByRoleId`](src/modules/models.js) — and **`importEstimationFieldKind`** / **`backlogRows[].estimationKind`** (import field type vs plan unit; invalid `estimationKind` cleared)).
...
*Last updated: `planningTimeMode` is authoritative for quarter vs sprint (set on create, sprint apply/remove, and reconciled in `init()`). Settings hides **Use Sprints Planning** / **Sprints settings** for quarter plans; `.sprint-planning-grid [hidden]` ensures the `hidden` attribute wins over flex labels. **Save Settings** only runs sprint teardown when the plan is in sprint mode. `bufferItems` and `#capacityBuffersBar` show active buffer names and percentages under the Capacity toolbar when buffers are on. Topbar plan export (`#planExportBtn`) is JSON/XLSX. Backlog and Capacity use bottom stats strips, checkbox bulk selection, and `.table-wrap` scrolling; Capacity uses member/role quick filter and no **Act** column. Sprint create flow replaces the anchor quarter with sprint periods; sprint **Working days** accepts multi-digit input with validation.*

...
*Last updated: resource grouping **by_member** (`targetCapacityRowId`, backlog/capacity wiring); **default % SP by roles** (`defaultRoleSplitPctByRoleId`, `validateAndCollectDefaultRoleSplitPct`, `distributeDefaultRoleSplitFromFirst`, `applyDefaultRoleSplitsToBacklogRows`); capacity SP + By team layout (`capacity-sp-by-team-layout`); manual team SP/day (`rowEstimationPerDayTeam`); static bilingual help (`public/help.html`); backlog bulk UI; capacity compact width; planning quarter/sprint layer.*

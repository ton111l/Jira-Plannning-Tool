export function cacheRefs(refs) {
  refs.createPlanBtn = document.getElementById("createPlanBtn");
  refs.planSelect = document.getElementById("planSelect");
  refs.settingsBtn = document.getElementById("settingsBtn");
  refs.teamNameInput = null;
  refs.tabButtons = Array.from(document.querySelectorAll(".tab"));
  refs.statusBar = document.getElementById("statusBar");
  refs.capacityWrapper = document.querySelector(".capacity-wrapper");
  refs.capacityPanel = document.getElementById("capacityPanel");
  refs.capacityTableWrap = document.getElementById("capacityTableWrap");
  refs.capacityOverlay = document.getElementById("capacityOverlay");
  refs.createPlanOverlayBtn = document.getElementById("createPlanOverlayBtn");
  refs.backlogPanel = document.getElementById("backlogPanel");
  refs.capacityTable = document.getElementById("capacityTable");
  refs.backlogTable = document.getElementById("backlogTable");
  refs.backlogTableWrap = document.getElementById("backlogTableWrap");
  refs.backlogOverlay = document.getElementById("backlogOverlay");
  refs.addCapacityRowBtn = document.getElementById("addCapacityRowBtn");
  refs.addQuarterBtn = document.getElementById("addQuarterBtn");
  refs.openImportModalBtn = document.getElementById("openImportModalBtn");
  refs.importOverlayBtn = document.getElementById("importOverlayBtn");
  refs.manualOverlayBtn = null;

  refs.createPlanDialog = document.getElementById("createPlanDialog");
  refs.createPlanForm = document.getElementById("createPlanForm");
  refs.planNameInput = document.getElementById("planNameInput");
  refs.quarterInput = document.getElementById("quarterInput");
  refs.yearInput = document.getElementById("yearInput");
  refs.createPlanEstimationTypeSelect = document.getElementById("createPlanEstimationTypeSelect");
  refs.createPlanResourceGroupingTypeSelect = document.getElementById("createPlanResourceGroupingTypeSelect");
  refs.createPeriodDialog = document.getElementById("createPeriodDialog");
  refs.createPeriodForm = document.getElementById("createPeriodForm");
  refs.periodQuarterInput = document.getElementById("periodQuarterInput");
  refs.periodYearInput = document.getElementById("periodYearInput");

  refs.importDialog = document.getElementById("importDialog");
  refs.importForm = document.getElementById("importForm");
  refs.importJiraBaseUrlInput = document.getElementById("importJiraBaseUrlInput");
  refs.importEstimationFieldModeWrap = document.getElementById("importEstimationFieldModeWrap");
  refs.importEstimationFieldModeInputs = Array.from(
    document.querySelectorAll("input[name='importEstimationFieldMode']")
  );
  refs.importEstimationFieldCustomWrap = document.getElementById("importEstimationFieldCustomWrap");
  refs.importEstimationFieldInput = document.getElementById("importEstimationFieldInput");
  refs.jqlInput = document.getElementById("jqlInput");
  refs.confirmImportBtn = document.getElementById("confirmImportBtn");
  refs.issuesCount = document.getElementById("issuesCount");
  refs.importProgress = document.getElementById("importProgress");

  refs.settingsDialog = document.getElementById("settingsDialog");
  refs.settingsForm = document.getElementById("settingsForm");
  refs.estimationTypeSelect = document.getElementById("estimationTypeSelect");
  refs.resourceGroupingTypeSelect = document.getElementById("resourceGroupingTypeSelect");
  refs.deleteConfirmDialog = document.getElementById("deleteConfirmDialog");
  refs.deleteConfirmForm = document.getElementById("deleteConfirmForm");
  refs.deleteConfirmText = document.getElementById("deleteConfirmText");
  refs.bulkWorkingDaysDialog = document.getElementById("bulkWorkingDaysDialog");
  refs.bulkWorkingDaysForm = document.getElementById("bulkWorkingDaysForm");
  refs.bulkWorkingDaysText = document.getElementById("bulkWorkingDaysText");
  refs.bulkWorkingDaysInput = document.getElementById("bulkWorkingDaysInput");
  refs.bulkRowEstimationDialog = document.getElementById("bulkRowEstimationDialog");
  refs.bulkRowEstimationForm = document.getElementById("bulkRowEstimationForm");
  refs.bulkRowEstimationText = document.getElementById("bulkRowEstimationText");
  refs.bulkRowEstimationModeWrap = document.getElementById("bulkRowEstimationModeWrap");
  refs.bulkRowEstimationManualWrap = document.getElementById("bulkRowEstimationManualWrap");
  refs.bulkRowEstimationInputLabel = document.getElementById("bulkRowEstimationInputLabel");
  refs.bulkRowEstimationModeInputs = Array.from(document.querySelectorAll("input[name='bulkRowEstimationMode']"));
  refs.bulkRowEstimationInput = document.getElementById("bulkRowEstimationInput");
  refs.bulkLoadDialog = document.getElementById("bulkLoadDialog");
  refs.bulkLoadForm = document.getElementById("bulkLoadForm");
  refs.bulkLoadPercentInput = document.getElementById("bulkLoadPercentInput");
}
